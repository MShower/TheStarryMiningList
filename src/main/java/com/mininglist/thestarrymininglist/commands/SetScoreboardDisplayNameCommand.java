                        ...
                        .then(argument("displayName", StringArgumentType.greedyString())
                                .executes(context -> {
                                    if (context.getSource().hasPermissionLevel(1)) {
                                        String scoreboardName = StringArgumentType.getString(context,
                                                "scoreboardName");
                                        String displayName = StringArgumentType.getString(context,
                                                "displayName");
                                        SetScoreboardDisplayName.set(scoreboardName,
                                                displayName,
                                                context.getSource());
                                        try {
                                            ...
                                            e.printStackTrace();
                                            Objects.requireNonNull(context.getSource().getPlayer()).sendMessage(Text.of("文件写入错误"),true);
                                        }
                                    }
                                    else {
                                        Objects.requireNonNull(context.getSource().getPlayer()).sendMessage(Text.of("权限不足"), true);
                                    }
                                    return 1;
                                })))
        );
